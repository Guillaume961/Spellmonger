...
import edu.insightr.spellmonger.model.cards.Rituol;
...
public class ControllerIA extends Application {
    ...
    public void start(final Stage primaryStage) {
        try {
            final URL url = getClass().getResource("/BoardIA.fxml");
    ...
    private boolean IaTurn(int i)
    {
        ...
        if (Model.GetSizeHand(1) < 5 && i==0)
        {
            ...
            if (!(toplay instanceof Creature))
            {
                for(Card tmpcard : cardia)
                {
                    if (tmpcard.getCost() > toplay.getCost() && tmpcard instanceof Rituol && tmpcard.getCost()<=Model.getCurrent().getEnergy())
                    {
                        toplay = tmpcard;
                    }
                }
            }
            if (!(toplay instanceof Creature) && !(toplay instanceof Rituol))
            {
                playNoCard();
                return false;
            }
            if(toplay instanceof Creature || toplay instanceof Rituol) {
                Image img = toplay.getImg();
    ...
    private void turnEnded2() {
        ...
        if(Model.getCurrent().equals(Model.getPlayer(2)))
        {
            IaTurn(0);
        }
    }
